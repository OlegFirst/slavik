...
async def lifespan(app: FastAPI):
    ...
    await optimizer.train_default_models()
...
class WorkflowOptimizerService:
    ...
    def load_models(self):
        ...
        try:
            ...
            for model_record in models:
                try:
                    # Deserialize model
                    # Decode from base64 first, then unpickle
                    decoded = base64.b64decode(model_record.model_data.encode('utf-8'))
    ...
    def _save_model_to_db(self, model_type: str, model, scaler, features: List[str], accuracy: float):
        ...
        try:
            ...
            # Encode to base64 to avoid NUL character issues in Postgres TEXT fields
            pickled = pickle.dumps(model_data)
            serialized_data = base64.b64encode(pickled).decode('utf-8')
...
async def metrics():
    ...
    Exposes AI workflow optimizer metrics for monitoring.
    """
...
@app.get("/api/v1/ai/analyze/{process_id}")
async def ai_analyze_workflow(process_id: str):
    """
    AI-powered workflow analysis using platform AI and experts

    Leverages:
    - AI Foundation for intelligent analysis
    - Expertise Center for BCM expert insights
    - Workflow Intelligence for pattern matching
    """
    global platform_client

    if not platform_client:
        raise HTTPException(status_code=503, detail="Platform client not initialized")
    ...
    try:
        ...
        # Ask AI for analysis
        ai_analysis = await platform_client.ai.ask(
            f"Analyze this workflow: {json.dumps(workflow_data)}. "
            "Provide optimization recommendations focusing on efficiency and resource usage."
        )

        # Get expert insights
        expert_insights = await platform_client.experts.query_expert(
            expert_type="process_optimizer",
            query=f"Review workflow {process_id} and suggest improvements",
            context=workflow_data
        )

        # Get similar cases from workflow intelligence
        similar_cases = await platform_client.workflows.search_cases({
            'module': 'process_optimization',
...
async def get_ai_recommendations(request: ProcessOptimizationRequest):
    """
    ...
    try:
        ...
        ai_recs = await platform_client.ai.ask(
            f"Given optimization goals {request.optimizationGoals}, "
            f"what are the top 3 recommendations for improving this workflow?",
            context=context
        )

        # Search knowledge base for best practices
        knowledge = await platform_client.ai.search_knowledge(
            f"workflow optimization {' '.join(request.optimizationGoals)}",
            limit=5
        )

        # Get workflow intelligence recommendations
        workflow_recs = await platform_client.workflows.get_recommendations(context)

        return {
            'process_id': request.processId,
            'ai_recommendations': ai_recs,
            'knowledge_base_insights': knowledge,
            'workflow_intelligence_recommendations': workflow_recs,
            'timestamp': datetime.utcnow().isoformat()
        }
...
async def learn_from_execution(execution_data: Dict[str, Any]):
    """
    Learn from workflow execution and add to case library
    ...
    try:
        ...
        # Determine if execution was successful enough to learn from
        success_rate = execution_data.get('success_rate', 0)

        if success_rate >= 0.8:  # Only learn from successful executions
            # Add to case library
            case_id = await platform_client.workflows.add_case(
                case_data=execution_data,
                module='workflow_optimization',
                source='ai_workflow_optimizer',
                metadata={
            ...
            logger.info(f"Learned from execution {process_id}, case ID: {case_id}")

            return {
                'success': True,
                'case_id': case_id,
                'message': f'Successfully learned from execution {process_id}'
            }
        else:
            return {
                'success': False,
                'message': f'Execution success rate ({success_rate}) below threshold (0.8)'
            }
...
async def platform_health():
    ...
    try:
        health_status = await platform_client.health_check()
