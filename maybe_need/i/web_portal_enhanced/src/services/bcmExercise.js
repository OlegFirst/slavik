...
class BCMExerciseService {
  ...
  async getExercises(filters = {}) {
    try {
      ...
      const exercises = await apiClient.searchRead(
        'bcm.exercise',
        domain,
        [
  ...
  async createExercise(exerciseData) {
    try {
      ...
      const exercise = await apiClient.create('bcm.exercise', payload)
  ...
  async updateExercise(exerciseId, updateData) {
    try {
      ...
      await apiClient.write('bcm.exercise', [exerciseId], payload)
  ...
  async getAvailableParticipants(companyId) {
    try {
      const users = await apiClient.searchRead(
        'res.users',
  ...
  async assignParticipants(exerciseId, participantAssignments) {
    try {
      ...
      for (const assignment of assignments) {
        await apiClient.create('bcm.exercise.participant', assignment)
      }
  ...
  async sendParticipantNotifications(exerciseId, assignments) {
    try {
      const exercise = await apiClient.read('bcm.exercise', [exerciseId], ['name', 'scheduled_date', 'description'])[0]

      for (const assignment of assignments) {
        const user = await apiClient.read('res.users', [assignment.user_id], ['name', 'email'])[0]
  ...
  async getScenarios(filters = {}) {
    try {
      ...
      const scenarios = await apiClient.searchRead(
        'bcm.scenario',
        domain,
        [
  ...
  async customizeScenario(scenarioId, customizations) {
    try {
      const baseScenario = await apiClient.read('bcm.scenario', [scenarioId])[0]
      ...
      const newScenario = await apiClient.create('bcm.scenario', customizedScenario)
      return newScenario
    } catch (error) {
  ...
  async startExercise(exerciseId) {
    try {
      ...
      await apiClient.write('bcm.exercise', [exerciseId], {
        status: 'in_progress',
        actual_start_time: new Date().toISOString()
      })

      // Initialize exercise session
      const session = await apiClient.create('bcm.exercise.session', {
        exercise_id: exerciseId,
      ...
      const exercise = await apiClient.read('bcm.exercise', [exerciseId])[0]
  ...
  async initializeSimulation(exerciseId, scenarioId) {
    try {
      const scenario = await apiClient.read('bcm.scenario', [scenarioId])[0]
  ...
  async getExerciseStatus(exerciseId) {
    try {
      const session = await apiClient.searchRead(
  ...
  async executeInject(exerciseId, injectData) {
    try {
      ...
      await apiClient.create('bcm.exercise.inject', {
        exercise_id: exerciseId,
      ...
      if (session) {
        ...
        await apiClient.write('bcm.exercise.session', [session.id], {
  ...
  async collectResults(exerciseId, resultsData) {
    try {
      const result = await apiClient.create('bcm.exercise.result', {
        exercise_id: exerciseId,
      ...
      await apiClient.write('bcm.exercise.result', [result.id], {
  ...
  async generateAnalysisReport(exerciseId) {
    try {
      const exercise = await apiClient.read('bcm.exercise', [exerciseId])[0]
      const results = await apiClient.searchRead(
        'bcm.exercise.result',
        [['exercise_id', '=', exerciseId]],
        ['*']
      )
      ...
      const reportRecord = await apiClient.create('bcm.exercise.analysis', {
        exercise_id: exerciseId,
  ...
  async getRuleBasedRecommendations(companyId, context) {
    try {
      ...
      const risks = await apiClient.searchRead('bcm.risk', [['company_id', '=', companyId]], ['risk_type', 'severity'])
