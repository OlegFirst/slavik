"""EventBus service integration for TheHive Adapter"""
...
class EventBusService:
    """EventBus integration service for TheHive Adapter"""
    ...
    async def publish(self, event_data: Dict[str, Any]):
        ...
        try:
            ...
            event_data['source'] = 'thehive-adapter'
            
            # Publish via HTTP API with retry logic
                ...
                try:
                    async with self.http_session.post(
                        ...
                        if response.status in [200, 201, 202]:
                            ...
                            logger.warning(f"Event publish failed (attempt {attempt + 1}): {response.status} - {error_text}")
                            
                except (aiohttp.ClientError, asyncio.TimeoutError) as e:
                    logger.warning(f"Event publish error (attempt {attempt + 1}): {str(e)}")
                
                if attempt < self.config.MAX_RETRIES - 1:
                    await asyncio.sleep(self.config.RETRY_DELAY * (2 ** attempt))
            
            logger.error(f"Failed to publish event after {self.config.MAX_RETRIES} attempts")
            return False
