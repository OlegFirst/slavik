...
# Configuration
class Config:
    LOG_DIR = os.getenv("LOG_DIR", "/tmp/bcm_logs")
    ...
    MONITORED_SERVICES = {
        ...
        "api_gateway": {"url": "http://unified_api_gateway:8777", "health": "/health", "type": "gateway"},
        "crm_bridge": {"url": "http://crm_bridge:8778", "health": "/health", "type": "bridge"},
        "postgres": {"url": "http://postgres:5432", "health": None, "type": "database"},
        "redis": {"url": "http://redis:6379", "health": None, "type": "cache"},
    }
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                if config["health"]:
                    ...
                    # For databases without HTTP health endpoints
...
async def get_dashboard():
                ...
                try {
                    ...
                    for (const [name, metrics] of Object.entries(services)) {
                        servicesHtml += `
                            <div class="card">
                                <h3>${name}</h3>
                                <p>Status: <span class="status-${metrics.status}">${metrics.status}</span></p>
    ...
    <body>
        <h1>🔍 BCM Platform Monitoring</h1>

        <div class="card">
            <h2>System Overview</h2>
            <p>Overall Status: <strong id="overall-status">Loading...</strong></p>
