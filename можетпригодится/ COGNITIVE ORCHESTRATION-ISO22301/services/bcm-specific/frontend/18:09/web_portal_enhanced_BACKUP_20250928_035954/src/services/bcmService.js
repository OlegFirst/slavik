...
class BCMService {
  ...
  async getKPIMetrics(companyId = null) {
    try {
      ...
      return await apiClient.searchRead('bcm.kpi', domain, ['name', 'value', 'target', 'status'])
    } catch (error) {
  ...
  async getTemplates(companyId = null) {
    try {
      const domain = companyId ? [['company_id', '=', companyId]] : []
      return await apiClient.searchRead('bcm.template', domain, ['name', 'template_type', 'version', 'status'])
    } catch (error) {
  ...
  async getScenarios(companyId = null) {
    try {
      const domain = companyId ? [['company_id', '=', companyId]] : []
      return await apiClient.searchRead('bcm.scenario', domain, ['name', 'scenario_type', 'complexity', 'rating'])
    } catch (error) {
  ...
  async getReports(companyId = null) {
    try {
      const domain = companyId ? [['company_id', '=', companyId]] : []
      return await apiClient.searchRead('bcm.report', domain, ['name', 'report_type', 'generate_date', 'status'])
    } catch (error) {
  ...
  async getTrainingStats(companyId = null) {
    try {
      const domain = companyId ? [['company_id', '=', companyId]] : []
      const training = await apiClient.searchRead('bcm.training', domain, ['status', 'completion_rate'])
  ...
  async getOrganizationalContext(companyId = null) {
    try {
      const domain = companyId ? [['company_id', '=', companyId]] : []
      return await apiClient.searchRead('bcm.context', domain, ['name', 'context_type', 'description', 'status'])
    } catch (error) {
  ...
  async getPortalData(userId) {
    try {
      return await apiClient.call('bcm.portal.dashboard', 'get_user_dashboard_data', [userId])
    } catch (error) {
  ...
  async getGovernanceItems(companyId = null) {
    try {
      const domain = companyId ? [['company_id', '=', companyId]] : []
      return await apiClient.searchRead('bcm.governance', domain, ['name', 'policy_type', 'approval_status', 'review_date'])
    } catch (error) {
  ...
  async getDashboardOverview(companyId = null) {
    try {
      ...
      const [incidents, risks, plans, exercises, audits] = await Promise.all([
        apiClient.searchRead('bcm.incident.management', companyId ? [['company_id', '=', companyId]] : [], ['status']),
        apiClient.searchRead('bcm.risk.management', companyId ? [['company_id', '=', companyId]] : [], ['risk_level']),
        apiClient.searchRead('bcm.plan', companyId ? [['company_id', '=', companyId]] : [], ['status']),
        apiClient.searchRead('bcm.exercise', companyId ? [['company_id', '=', companyId]] : [], ['status']),
        apiClient.searchRead('bcm.audit', companyId ? [['company_id', '=', companyId]] : [], ['status'])
      ])
