...
class BaseOrchestrator(ABC):
    ...
    Must be extended by specific orchestrator implementations.
    Dependencies are injected to allow for flexible composition.
    """
    ...
    def __init__(self,
        ...
        Args:
            ...
            event_coordinator: EventCoordinator instance (optional)
            health_monitor: HealthMonitor instance (optional)
            docker_manager: DockerManager instance (optional)
        """
    ...
    async def publish_event(self, event_type: str, data: Dict[str, Any],
        ...
        if not self.event_coordinator:
            logger.warning(f"No event coordinator configured for {self.name}")
            return
    ...
    async def register_service(self, service_name: str,
        ...
        if not self.service_registry:
            logger.warning(f"No service registry configured for {self.name}")
            return
    ...
    async def monitor_service_health(self, service_name: str) -> bool:
        ...
        if not self.health_monitor:
            logger.warning(f"No health monitor configured for {self.name}")
            return False

        try:
            status = await self.health_monitor.check_service(service_name)
            return status.healthy if hasattr(status, 'healthy') else status.is_healthy()
        except Exception as e:
    ...
    async def start_docker_service(self, service_name: str,
        ...
        if not self.docker_manager:
            logger.error(f"No docker manager configured for {self.name}")
            return False

        try:
            success = await self.docker_manager.start_service(service_name, timeout=timeout)
            if success:
                # Wait for healthy
