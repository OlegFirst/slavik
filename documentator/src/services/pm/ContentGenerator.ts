...
export class ContentGenerator {
  private prompts: Map<string, ContentPrompt> = new Map();
  ...
  validateFieldData(field: PromptField, value: any): { valid: boolean; error?: string } {
  ...
  validateRequest(request: ContentRequest): { valid: boolean; errors: string[] } {
  ...
  private async processPrompt(prompt: ContentPrompt, data: Record<string, any>): Promise<string> {
  ...
  private generateMockContent(prompt: ContentPrompt, data: Record<string, any>): string {
  ...
  private generatePressReleaseMock(data: Record<string, any>): string {
...
*Цей прес-реліз згенеровано Digital Office PM з потужними AI промптами для максимального впливу на ${data.targetAudience}.*`;
  ...
  private generateProjectPointMock(data: Record<string, any>): string {
    const date = new Date().toLocaleDateString('uk-UA');
...
*Повідомлення створено Digital Office PM для ${data.stakeholders || 'команди проекту'}*`;
  ...
  private getAnnouncementTypeText(type: string): string {
  ...
  private generateSubheadline(data: Record<string, any>): string {
    return `Нова веха у розвитку для ${data.targetAudience}`;
  }

  private generateAchievementTitle(achievement: string): string {
  ...
  private async saveContent(content: string, filePath: string, format: string): Promise<void> {
