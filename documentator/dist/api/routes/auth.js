...
function authRoutes(authService) {
    ...
    const authMiddleware = new middleware_1.AuthMiddleware(authService);
    ...
    router.post('/api-key', authMiddleware.authenticate, async (req, res) => {
        try {
    ...
    router.put('/users/:userId/role', authMiddleware.authenticate, authMiddleware.requireRole('admin'), async (req, res) => {
        try {
