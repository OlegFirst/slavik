...
try:
    ...
    class ExecutionMetricsContext:
        ...
        def __exit__(self, *args):
            return False
    def record_scenario_executed(*args, **kwargs):
        pass
    def record_eventbus_event_published(*args, **kwargs):
        pass
...
class ExecutionEngine:
    ...
    async def execute_scenario_direct(
        ...
        Returns:
            ExecutionReport
        """
        # Get scenario level for metrics
        scenario_level = scenario.get('meta', {}).get('level', 1)
        level_str = f'l{scenario_level}'
        ...
        with ExecutionMetricsContext(level=level_str):
            ...
            # Record metrics
            steps_successful = sum(1 for s in result.steps if s.status == 'success')
