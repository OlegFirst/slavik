...
class InfrastructureCoordinator:
    ...
    def __init__(self, event_bus_backend='redis', redis_url=None, enable_governance=True):
        ...
        import os
        health_monitor_path = os.path.join(
            os.path.dirname(__file__),
            '../../../intelligent_core/orchestration/ai_orchestration/core/health_monitor.py'
        )
        ...
        if enable_governance:
            ...
            policy_path = os.path.join(
                os.path.dirname(__file__),
                '../../policy_engine/policies.yaml'
            )
            if os.path.exists(policy_path):
                policy_engine = initialize_policy_engine(policy_path)
                ...
                policy_engine = initialize_policy_engine()  # Use defaults

            # Create Decision Center (central governance) with policy_engine
            from infrastructure.policy_engine import get_policy_engine
            self.decision_center = InfrastructureDecisionCenter(
                eventbus=self.eventbus,
                policy_engine=get_policy_engine()  # Pass global policy engine
            )
