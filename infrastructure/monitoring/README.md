...
Service will start on **http://localhost:8045**
...
```javascript
const ws = new WebSocket('ws://localhost:8045/ws/realtime');
...
# Push log
await httpx.post("http://localhost:8045/logs", json={
    "service": "my_service",
    "level": "ERROR",
...
# Push metrics
await httpx.post("http://localhost:8045/metrics", json={
    "service": "my_service",
    "status": "healthy",
    "response_time_ms": 125.5
})
...
### Test Log Ingestion:
```bash
curl -X POST http://localhost:8045/logs \
...
### Test System Status:
```bash
curl http://localhost:8045/status
```
...
docker run -p 8045:8045 --env-file .env monitoring-service
```
