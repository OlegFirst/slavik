...
class NotificationChannel(BaseModel):
    ...
    @validator('recipients', 'webhook_url', 'integration_key')
    def validate_required_if_enabled(cls, v, values, field):
        """If channel is enabled, ensure required fields are present"""
        if values.get('enabled', False):
            if field.name in ['recipients', 'webhook_url', 'integration_key']:
                ...
                if not has_config and field.name == 'recipients':
                    raise ValueError(
                        "Enabled notification channel must have recipients, webhook_url, or integration_key"
                    )
        return v
