...
class AutomationToolkitManager:
    ...
    async def generate_prometheus_config(self, services: List[Dict]) -> Dict:
        """
        Автоматическая генерация Prometheus scrape configs

        Args:
            services: Список сервисов с /metrics endpoints

        Returns:
            {
                'scrape_configs': [...],
                'config_file': 'path/to/prometheus-jobs-auto.yml',
                'services_count': N
            }
        """
        ...
        for service in services_with_metrics:
            # Extract port from service name or endpoints
            port = self._extract_port(service)
        ...
        # Save to YAML file
        import yaml
        config_dir = Path(__file__).parent.parent / 'generated_configs'
        config_dir.mkdir(exist_ok=True)

        config_file = config_dir / 'prometheus-jobs-auto.yml'
        ...
        return {
            ...
            'services_count': len(scrape_configs),
            'timestamp': datetime.utcnow().isoformat()
        }

    async def generate_grafana_dashboard(self, services: List[Dict]) -> Dict:
        """
        Автоматическая генерация Grafana dashboard
        ...
        Returns:
            {
                'dashboard_file': 'path/to/dashboard.json',
                'panels_count': N
            }
        """
        ...
        # Dashboard structure
        dashboard = {
            "title": "MIO Manager - Auto-generated Services Dashboard",
            "tags": ["mio-manager", "auto-generated"],
            "timezone": "browser",
            "panels": [],
            "refresh": "30s",
            "time": {
        ...
        # Generate panels for each service
        panel_id = 1
        y_pos = 0
        ...
        for service in services_with_metrics:
            ...
            dashboard['panels'].append({
                "id": panel_id,
                "title": f"{service_name} - Response Time (p95)",
                "type": "graph",
            ...
            dashboard['panels'].append({
                ...
                "type": "graph",
                "gridPos": {"h": 8, "w": 12, "x": 0, "y": y_pos + 8},
                "targets": [{
            ...
            # Panel 4: CPU/Memory (if available)
            dashboard['panels'].append({
                "id": panel_id,
                "title": f"{service_name} - Resource Usage",
                "type": "graph",
                "gridPos": {"h": 8, "w": 12, "x": 12, "y": y_pos + 8},
                "targets": [
                    {
                        "expr": f'process_cpu_seconds_total{{service="{service_name}"}}',
                        "legendFormat": "CPU"
                    },
                    {
        ...
        dashboard['panels'].insert(0, {
            ...
            "targets": [{
                "expr": 'count(up{managed_by="mio-manager"} == 1)',
        ...
        dashboard_file = config_dir / 'grafana-dashboard-auto.json'
        ...
        return {
            ...
            'services_count': len(services_with_metrics),
            'timestamp': datetime.utcnow().isoformat()
        }
    ...
    async def setup_monitoring(self) -> Dict:
        ...
        prometheus_config = await self.generate_prometheus_config(services)

        # 3. Generate Grafana dashboard
        grafana_dashboard = await self.generate_grafana_dashboard(services)

        result = {
            'status': 'completed',
            'services_discovered': discovery['total_services'],
            'services_with_metrics': len([s for s in services if s.get('has_metrics')]),
            'coverage_percentage': discovery['coverage']['percentage'],
            'prometheus_config': prometheus_config['config_file'],
            'grafana_dashboard': grafana_dashboard['dashboard_file'],
            'prometheus_scrape_jobs': prometheus_config['services_count'],
            'grafana_panels': grafana_dashboard['panels_count'],
            'timestamp': datetime.utcnow().isoformat()
        }
    ...
    def _extract_port(self, service: Dict) -> int:
        """
        Extract port from service info

        Известные порты сервисов:
        """
        service_name = service['name']

        # Mapping известных сервисов к портам
        port_mapping = {
            ...
            'mio-manager': 8046,
            'workflow-intelligence': 8050,
        }

        # Если знаем порт - возвращаем
        if service_name in port_mapping:
            return port_mapping[service_name]

        # Попытка извлечь из endpoints
