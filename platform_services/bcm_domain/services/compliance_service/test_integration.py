"""
Test Integration - Compliance Service
=====================================
...
async def test_platform_integration():
    ...
    print("\n" + "=" * 70)
    print("Testing Compliance Service Platform Integration")
    print("=" * 70 + "\n")
    ...
    try:
        platform = await init_platform(
            database_url="sqlite+aiosqlite:///./test_compliance_service.db",
    ...
    try:
        ...
        class TestComplianceService(SelfAwareService):
            def __init__(self):
                super().__init__(
                    service_name="compliance_service_test",
                    capabilities=["compliance.assessment", "compliance.audit"]
                )

            async def handle_test_event(self, event):
                return {"status": "handled", "event_type": event.type}

        test_service = TestComplianceService()

        test_service.register_handler("compliance.test.*", test_service.handle_test_event, priority=EventPriority.HIGH)
    ...
    if platform.intelligent_router:
        ...
        try:
            ...
            test_event = Event.create(
                event_type="compliance.integration.test",
                data={"message": "Integration test event for Compliance Service"},
...
async def main():
    ...
    try:
        ...
        if success:
            ...
            print("   2. Test API endpoints: http://localhost:8030/docs")
            print("   3. Check health: curl http://localhost:8030/health")
            print("   4. Check platform status: curl http://localhost:8030/api/platform/status")
            return 0
        else:
