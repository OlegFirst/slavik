"""
Test Integration - Documents Service
====================================
...
async def test_platform_integration():
    ...
    print("\n" + "=" * 70)
    print("Testing Documents Service Platform Integration")
    print("=" * 70 + "\n")
    ...
    try:
        platform = await init_platform(
            database_url="sqlite+aiosqlite:///./test_documents_service.db",
    ...
    try:
        ...
        class TestDocumentsService(SelfAwareService):
            def __init__(self):
                super().__init__(
                    service_name="documents_service_test",
                    capabilities=["documents.create", "documents.version_control"]
                )

            async def handle_test_event(self, event):
                return {"status": "handled", "event_type": event.type}

        test_service = TestDocumentsService()

        test_service.register_handler("documents.test.*", test_service.handle_test_event, priority=EventPriority.HIGH)
    ...
    if platform.intelligent_router:
        ...
        try:
            ...
            test_event = Event.create(
                event_type="documents.integration.test",
                data={"message": "Integration test event for Documents Service"},
...
async def main():
    ...
    try:
        ...
        if success:
            ...
            print("   2. Test API endpoints: http://localhost:8090/docs")
            print("   3. Check health: curl http://localhost:8090/health")
            print("   4. Check platform status: curl http://localhost:8090/api/platform/status")
            return 0
        else:
