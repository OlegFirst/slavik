"""
Advanced Health Checks for Planning Service
...
async def check_orchestrator() -> Dict[str, Any]:
    """Check Orchestrator connectivity (optional)"""
    try:
        async with httpx.AsyncClient(timeout=2.0) as client:
            response = await client.get(f"{settings.ORCHESTRATOR_URL}/health")

        if response.status_code == 200:
            return {"status": "healthy", "details": "Orchestrator reachable"}
        else:
            return {"status": "degraded", "status_code": response.status_code}
    except Exception as e:
        # Orchestrator is optional - degraded, not unhealthy
        return {
            "status": "degraded",
            "details": "Orchestrator unavailable (optional service)"
        }
...
async def detailed_health_check():
    ...
    orchestrator_task = asyncio.create_task(check_orchestrator())

    # Wait for all checks
    checks["database"] = await database_task
    checks["eventbus"] = await eventbus_task
    checks["orchestrator"] = await orchestrator_task
