"""
Planning Service - Integrated with Graceful Choreography
=========================================================

Enhanced Planning Service with:
...
class PlanningServiceSelfAware(SelfAwareService if SELF_AWARE_AVAILABLE else object):
    """
    Planning Service with Self-Aware capabilities
    ...
    def __init__(self):
        if SELF_AWARE_AVAILABLE:
            ...
            logger.info("✅ Planning Service initialized as Self-Aware")
        else:
            logger.warning("⚠️  Planning Service running in basic mode (Self-Aware not available)")

    async def initialize(self):
        """Initialize Planning Service-specific capabilities"""
        if SELF_AWARE_AVAILABLE:
            # Register event handlers with priorities
            self.register_handler("planning.strategy.*", self.handle_strategy_event, priority=EventPriority.HIGH)
            self.register_handler("planning.objectives.*", self.handle_objectives_event, priority=EventPriority.HIGH)
            self.register_handler("planning.resource_planning.*", self.handle_resource_planning_event, priority=EventPriority.NORMAL)
            self.register_handler("planning.scheduling.*", self.handle_scheduling_event, priority=EventPriority.NORMAL)

            logger.info("✅ Planning Service event handlers registered")

    async def handle_strategy_event(self, event):
        """Handle planning.strategy events"""
        logger.info(f"Handling planning.strategy event: {event.type}")
    ...
    async def handle_objectives_event(self, event):
        """Handle planning.objectives events"""
        logger.info(f"Handling planning.objectives event: {event.type}")
        # TODO: Implement actual objectives handling
        return {"status": "processed", "event_type": event.type}
    async def handle_resource_planning_event(self, event):
        """Handle planning.resource_planning events"""
        logger.info(f"Handling planning.resource_planning event: {event.type}")
        # TODO: Implement actual resource_planning handling
        return {"status": "processed", "event_type": event.type}
    async def handle_scheduling_event(self, event):
        """Handle planning.scheduling events"""
        logger.info(f"Handling planning.scheduling event: {event.type}")
        # TODO: Implement actual scheduling handling
        return {"status": "processed", "event_type": event.type}

    async def handle_completion_event(self, event):
        """Handle completion events (triggers next steps)"""
        logger.info(f"Planning Service completion event: {event.type}")
...
async def lifespan(app: FastAPI):
    ...
    # === STARTUP ===
    logger.info("=" * 60)
    logger.info("🚀 Starting Planning Service (Integrated with Graceful Choreography)")
    logger.info("=" * 60)
    logger.info(f"📍 Port: 8050")
    logger.info(f"📋 ISO 22301 Clause: 8.3")
    ...
    try:
        ...
        if SELF_AWARE_AVAILABLE:
            ...
            service_self_aware = PlanningServiceSelfAware()
            ...
            if platform.intelligent_router:
                ...
                await platform.intelligent_router.register_subscriber(
                    subscriber_id="planning_service",
                    event_pattern="planning.*",
                    handler=service_self_aware.on_event,
                    capabilities={
                        "domains": ["planning", "bcm"],
                        "max_concurrent": 10,
                ...
                await platform.eventbus.subscribe("planning.*", service_self_aware.on_event)
        ...
        logger.info("\n" + "=" * 60)
        logger.info("✅ Planning Service ready (with Graceful Choreography)")
        logger.info("=" * 60)
        logger.info(f"   🌐 Docs: http://localhost:8050/docs")
        logger.info(f"   📊 Metrics: http://localhost:8050/metrics")
        logger.info(f"   ❤️  Health: http://localhost:8050/health")
        logger.info("=" * 60 + "\n")

    except Exception as e:
        logger.error(f"❌ Failed to start Planning Service: {e}", exc_info=True)
        raise
    ...
    logger.info("=" * 60)
    logger.info("✅ Planning Service shutdown complete")
    logger.info("=" * 60)


# Create FastAPI app with enhanced lifespan
app = FastAPI(
    title=getattr(settings, 'SERVICE_NAME', 'Planning Service'),
    description=f"""
    BCM Planning - ISO 22301 Clause 8.3
