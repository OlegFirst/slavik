"""
Workflow Intelligence Integration for Planning Service
...
class WorkflowSecurityMiddleware:
    """
    Security middleware for Planning Service
...
async def check_planning_compliance(
    workflow_context: Dict[str, Any],
    iso_checker: ISO22301Checker
) -> Dict[str, Any]:
    """
    Check ISO 22301 Clause 8.3 compliance for planning
    ...
    # Check compliance for planning (Clause 8.3)
    ...
    logger.info(
        f"ISO 22301 Clause 8.3 compliance: {result['compliance_percentage']:.1f}%"
    )

    if not result['compliant']:
        logger.warning(
            f"Planning compliance gaps found: {len(result['gaps'])} issues"
        )
