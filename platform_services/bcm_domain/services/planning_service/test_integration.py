"""
Test Integration - Planning Service
===================================
...
async def test_platform_integration():
    ...
    print("\n" + "=" * 70)
    print("Testing Planning Service Platform Integration")
    print("=" * 70 + "\n")
    ...
    try:
        platform = await init_platform(
            database_url="sqlite+aiosqlite:///./test_planning_service.db",
    ...
    try:
        ...
        class TestPlanningService(SelfAwareService):
            def __init__(self):
                super().__init__(
                    service_name="planning_service_test",
                    capabilities=["planning.strategy", "planning.objectives"]
                )

            async def handle_test_event(self, event):
                return {"status": "handled", "event_type": event.type}

        test_service = TestPlanningService()

        test_service.register_handler("planning.test.*", test_service.handle_test_event, priority=EventPriority.HIGH)
    ...
    if platform.intelligent_router:
        ...
        try:
            ...
            test_event = Event.create(
                event_type="planning.integration.test",
                data={"message": "Integration test event for Planning Service"},
...
async def main():
    ...
    try:
        ...
        if success:
            ...
            print("   2. Test API endpoints: http://localhost:8050/docs")
            print("   3. Check health: curl http://localhost:8050/health")
            print("   4. Check platform status: curl http://localhost:8050/api/platform/status")
            return 0
        else:
