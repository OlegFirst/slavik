"""
Test Integration - Community Service
====================================
...
async def test_platform_integration():
    ...
    print("\n" + "=" * 70)
    print("Testing Community Service Platform Integration")
    print("=" * 70 + "\n")
    ...
    try:
        platform = await init_platform(
            database_url="sqlite+aiosqlite:///./test_community_service.db",
    ...
    try:
        ...
        class TestCommunityService(SelfAwareService):
            def __init__(self):
                super().__init__(
                    service_name="community_service_test",
                    capabilities=["community.knowledge_sharing", "community.collaboration"]
                )

            async def handle_test_event(self, event):
                return {"status": "handled", "event_type": event.type}

        test_service = TestCommunityService()

        test_service.register_handler("community.test.*", test_service.handle_test_event, priority=EventPriority.HIGH)
    ...
    if platform.intelligent_router:
        ...
        try:
            ...
            test_event = Event.create(
                event_type="community.integration.test",
                data={"message": "Integration test event for Community Service"},
...
async def main():
    ...
    try:
        ...
        if success:
            ...
            print("   2. Test API endpoints: http://localhost:8120/docs")
            print("   3. Check health: curl http://localhost:8120/health")
            print("   4. Check platform status: curl http://localhost:8120/api/platform/status")
            return 0
        else:
