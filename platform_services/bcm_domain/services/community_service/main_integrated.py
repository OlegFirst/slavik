"""
Community Service - Integrated with Graceful Choreography
==========================================================

Enhanced Community Service with:
...
class CommunityServiceSelfAware(SelfAwareService if SELF_AWARE_AVAILABLE else object):
    """
    Community Service with Self-Aware capabilities
    ...
    def __init__(self):
        if SELF_AWARE_AVAILABLE:
            super().__init__(
                service_name="community_service",
                capabilities=[
            ...
            logger.info(" Community Service initialized as Self-Aware")
        else:
            logger.warning("️  Community Service running in basic mode (Self-Aware not available)")

    async def initialize(self):
        """Initialize Community Service-specific capabilities"""
        if SELF_AWARE_AVAILABLE:
            # Register event handlers with priorities
            self.register_handler("community.knowledge_sharing.*", self.handle_knowledge_sharing_event, priority=EventPriority.HIGH)
            self.register_handler("community.collaboration.*", self.handle_collaboration_event, priority=EventPriority.HIGH)
            self.register_handler("community.marketplace.*", self.handle_marketplace_event, priority=EventPriority.NORMAL)
            self.register_handler("community.portal.*", self.handle_portal_event, priority=EventPriority.NORMAL)

            logger.info(" Community Service event handlers registered")

    async def handle_knowledge_sharing_event(self, event):
        """Handle community.knowledge_sharing events"""
        logger.info(f"Handling community.knowledge_sharing event: {event.type}")
        # TODO: Implement actual knowledge_sharing handling
        return {"status": "processed", "event_type": event.type}
    async def handle_collaboration_event(self, event):
        """Handle community.collaboration events"""
        logger.info(f"Handling community.collaboration event: {event.type}")
        # TODO: Implement actual collaboration handling
        return {"status": "processed", "event_type": event.type}
    async def handle_marketplace_event(self, event):
        """Handle community.marketplace events"""
        logger.info(f"Handling community.marketplace event: {event.type}")
        # TODO: Implement actual marketplace handling
        return {"status": "processed", "event_type": event.type}
    async def handle_portal_event(self, event):
        """Handle community.portal events"""
        logger.info(f"Handling community.portal event: {event.type}")
        # TODO: Implement actual portal handling
        return {"status": "processed", "event_type": event.type}

    async def handle_completion_event(self, event):
        """Handle completion events (triggers next steps)"""
        logger.info(f"Community Service completion event: {event.type}")
...
async def lifespan(app: FastAPI):
    ...
    # === STARTUP ===
    logger.info("=" * 60)
    logger.info(" Starting Community Service (Integrated with Graceful Choreography)")
    logger.info("=" * 60)
    logger.info(f" Port: 8120")
    logger.info(f" ISO 22301 Clause: Support")
    ...
    try:
        ...
        if SELF_AWARE_AVAILABLE:
            ...
            service_self_aware = CommunityServiceSelfAware()
            ...
            if platform.intelligent_router:
                ...
                await platform.intelligent_router.register_subscriber(
                    subscriber_id="community_service",
                    event_pattern="community.*", "knowledge.*",
                    handler=service_self_aware.on_event,
                    capabilities={
                        "domains": ["community", "bcm"],
                        "max_concurrent": 10,
                ...
                await platform.eventbus.subscribe("community.*", "knowledge.*", service_self_aware.on_event)
        ...
        logger.info("\n" + "=" * 60)
        logger.info(" Community Service ready (with Graceful Choreography)")
        logger.info("=" * 60)
        logger.info(f"    Docs: http://localhost:8120/docs")
        logger.info(f"    Metrics: http://localhost:8120/metrics")
        logger.info(f"   ️  Health: http://localhost:8120/health")
        logger.info("=" * 60 + "\n")

    except Exception as e:
        logger.error(f" Failed to start Community Service: {e}", exc_info=True)
        raise
    ...
    logger.info("=" * 60)
    logger.info(" Community Service shutdown complete")
    logger.info("=" * 60)


# Create FastAPI app with enhanced lifespan
app = FastAPI(
    title=getattr(settings, 'SERVICE_NAME', 'Community Service'),
    description=f"""
