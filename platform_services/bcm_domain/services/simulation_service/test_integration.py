"""
Test Integration - Simulation Service
=====================================
...
async def test_platform_integration():
    ...
    print("\n" + "=" * 70)
    print("Testing Simulation Service Platform Integration")
    print("=" * 70 + "\n")
    ...
    try:
        platform = await init_platform(
            database_url="sqlite+aiosqlite:///./test_simulation_service.db",
    ...
    try:
        ...
        class TestSimulationService(SelfAwareService):
            def __init__(self):
                super().__init__(
                    service_name="simulation_service_test",
                    capabilities=["simulation.scenario", "simulation.monte_carlo"]
                )

            async def handle_test_event(self, event):
                return {"status": "handled", "event_type": event.type}

        test_service = TestSimulationService()

        test_service.register_handler("simulation.test.*", test_service.handle_test_event, priority=EventPriority.HIGH)
    ...
    if platform.intelligent_router:
        ...
        try:
            ...
            test_event = Event.create(
                event_type="simulation.integration.test",
                data={"message": "Integration test event for Simulation Service"},
...
async def main():
    ...
    try:
        ...
        if success:
            ...
            print("   2. Test API endpoints: http://localhost:8130/docs")
            print("   3. Check health: curl http://localhost:8130/health")
            print("   4. Check platform status: curl http://localhost:8130/api/platform/status")
            return 0
        else:
