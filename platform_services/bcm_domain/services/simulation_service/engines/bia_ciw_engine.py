...
Adapted from: /simulation/bia_engine/bia_ciw_engine.py
"""
...
try:
    ...
    logger.warning("Ciw library not available - queue simulation features disabled")
    CIW_AVAILABLE = False
...
class BCMQueueSimulator:
    ...
    def __init__(self):
        ...
        if not CIW_AVAILABLE:
            logger.warning("Queue simulator initialized without Ciw support")
    ...
    def create_business_process_network(
        ...
        if not CIW_AVAILABLE:
            raise RuntimeError("Ciw library not installed")
    ...
    def simulate_process_disruption(
        ...
        if not CIW_AVAILABLE:
            return self._mock_disruption_simulation(
                process_name,
        ...
        def calculate_metrics(records):
            ...
            return {
                ...
                'queue_length': np.mean([r.queue_size_at_arrival for r in records
                                        if hasattr(r, 'queue_size_at_arrival')]) if records else 0,
    ...
    def calculate_rto_rpo_optimal(
        ...
        if utilization >= 1:
            ...
            rpo = min(max_data_loss_hours, 1 / max(transactions_per_hour, 0.01))
    ...
    def _calculate_mm_c_wait_time(self, λ: float, μ: float, c: int) -> float:
        ...
        # For full implementation, use scipy or mpmath
        w = ρ / (c * μ * (1 - ρ))
    ...
    def _mock_disruption_simulation(
        self,
        process_name: str,
        disruption_duration_hours: float,
        disruption_severity: float
    ) -> Dict[str, Any]:
        """Mock simulation when Ciw is not available"""
        return {
            'process_name': process_name,
            'disruption_severity': disruption_severity,
            'disruption_duration': disruption_duration_hours,
            'wait_time_increase_pct': disruption_severity * 200,
            'throughput_decrease_pct': disruption_severity * 80,
            'queue_length_increase': disruption_severity * 10,
            'normal_metrics': {
                'avg_wait_time': 5.0,
                'throughput': 20.0,
                'queue_length': 2.0
            ...
            'disrupted_metrics': {
                'avg_wait_time': 5.0 * (1 + disruption_severity * 2),
                'throughput': 20.0 * (1 - disruption_severity * 0.8),
                'queue_length': 2.0 + disruption_severity * 10
            },
            'note': 'Mock simulation - Ciw library not installed'
        }
...
class AdvancedBIAEngine:
    ...
    def comprehensive_bia_analysis(
        ...
        if CIW_AVAILABLE:
            ...
            rto_rpo = {
                'optimal_rto_hours': 4.0,
                'optimal_rpo_hours': 1.0,
                'utilization': 0.7,
                'stability': 'unknown',
                'recommendations': ['Install Ciw library for detailed analysis']
            }
    ...
    def _summarize_financial_impacts(self, scenario_impacts: List[Dict]) -> Dict:
        ...
        return {
            'min_impact': round(min(total_impacts) if total_impacts else 0, 2),
            'max_impact': round(max(total_impacts) if total_impacts else 0, 2),
            'avg_impact': round(np.mean(total_impacts) if total_impacts else 0, 2),
    ...
    def _generate_bia_recommendations(
        ...
        max_impact = max([s.get('financial_impact', {}).get('total_impact', 0)
                         for s in scenario_impacts] or [0])
