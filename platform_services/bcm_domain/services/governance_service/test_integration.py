"""
Test Integration - Governance Service
=====================================
...
async def test_platform_integration():
    ...
    print("\n" + "=" * 70)
    print("Testing Governance Service Platform Integration")
    print("=" * 70 + "\n")
    ...
    try:
        platform = await init_platform(
            database_url="sqlite+aiosqlite:///./test_governance_service.db",
    ...
    try:
        ...
        class TestGovernanceService(SelfAwareService):
            def __init__(self):
                super().__init__(
                    service_name="governance_service_test",
                    capabilities=["governance.policy", "governance.roles"]
                )

            async def handle_test_event(self, event):
                return {"status": "handled", "event_type": event.type}

        test_service = TestGovernanceService()

        test_service.register_handler("governance.test.*", test_service.handle_test_event, priority=EventPriority.HIGH)
    ...
    if platform.intelligent_router:
        ...
        try:
            ...
            test_event = Event.create(
                event_type="governance.integration.test",
                data={"message": "Integration test event for Governance Service"},
...
async def main():
    ...
    try:
        ...
        if success:
            ...
            print("   2. Test API endpoints: http://localhost:8060/docs")
            print("   3. Check health: curl http://localhost:8060/health")
            print("   4. Check platform status: curl http://localhost:8060/api/platform/status")
            return 0
        else:
