"""
Governance Service - Integrated with Graceful Choreography
===========================================================

Enhanced Governance Service with:
...
class GovernanceServiceSelfAware(SelfAwareService if SELF_AWARE_AVAILABLE else object):
    """
    Governance Service with Self-Aware capabilities
    ...
    async def handle_completion_event(self, event):
        ...
        logger.info(f"Governance Service completion event: {event.type}")
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if SELF_AWARE_AVAILABLE:
            ...
            service_self_aware = GovernanceServiceSelfAware()
...
app = FastAPI(
    title=getattr(settings, 'SERVICE_NAME', 'Governance Service'),
    description=f"""
    Governance & Leadership - ISO 22301 Clauses 5.1-5.3
