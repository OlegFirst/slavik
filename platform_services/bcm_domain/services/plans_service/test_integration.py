"""
Test Integration - Plans Service
================================
...
async def test_platform_integration():
    ...
    print("\n" + "=" * 70)
    print("Testing Plans Service Platform Integration")
    print("=" * 70 + "\n")
    ...
    try:
        platform = await init_platform(
            database_url="sqlite+aiosqlite:///./test_plans_service.db",
    ...
    try:
        ...
        class TestPlansService(SelfAwareService):
            def __init__(self):
                super().__init__(
                    service_name="plans_service_test",
                    capabilities=["plans.bcp_create", "plans.bcp_update"]
                )

            async def handle_test_event(self, event):
                return {"status": "handled", "event_type": event.type}

        test_service = TestPlansService()

        test_service.register_handler("plans.test.*", test_service.handle_test_event, priority=EventPriority.HIGH)
    ...
    if platform.intelligent_router:
        ...
        try:
            ...
            test_event = Event.create(
                event_type="plans.integration.test",
                data={"message": "Integration test event for Plans Service"},
...
async def main():
    ...
    try:
        ...
        if success:
            ...
            print("   2. Test API endpoints: http://localhost:8070/docs")
            print("   3. Check health: curl http://localhost:8070/health")
            print("   4. Check platform status: curl http://localhost:8070/api/platform/status")
            return 0
        else:
