"""
Plans Service - Integrated with Graceful Choreography
======================================================

Enhanced Plans Service with:
...
class PlansServiceSelfAware(SelfAwareService if SELF_AWARE_AVAILABLE else object):
    """
    Plans Service with Self-Aware capabilities
    ...
    def __init__(self):
        if SELF_AWARE_AVAILABLE:
            ...
            logger.warning("️  Plans Service running in basic mode (Self-Aware not available)")
    ...
    async def handle_completion_event(self, event):
        ...
        logger.info(f"Plans Service completion event: {event.type}")
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if SELF_AWARE_AVAILABLE:
            ...
            service_self_aware = PlansServiceSelfAware()
            ...
            if platform.intelligent_router:
                ...
                await platform.eventbus.subscribe("plans.*", service_self_aware.on_event)
        ...
        logger.error(f" Failed to start Plans Service: {e}", exc_info=True)
        raise
    ...
    # === SHUTDOWN ===
    logger.info("\n" + "=" * 60)
    logger.info(" Shutting down Plans Service...")
    logger.info("=" * 60)
    ...
    logger.info("=" * 60)
    logger.info(" Plans Service shutdown complete")
    logger.info("=" * 60)


# Create FastAPI app with enhanced lifespan
app = FastAPI(
    title=getattr(settings, 'SERVICE_NAME', 'Plans Service'),
    description=f"""
    BCP/DRP Management - ISO 22301 Clause 8.4
