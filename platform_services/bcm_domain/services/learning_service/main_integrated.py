"""
Learning Service - Integrated with Graceful Choreography
=========================================================

Enhanced Learning Service with:
...
class LearningServiceSelfAware(SelfAwareService if SELF_AWARE_AVAILABLE else object):
    """
    Learning Service with Self-Aware capabilities
    ...
    def __init__(self):
        if SELF_AWARE_AVAILABLE:
            ...
            logger.info("✅ Learning Service initialized as Self-Aware")
        else:
            logger.warning("⚠️  Learning Service running in basic mode (Self-Aware not available)")

    async def initialize(self):
        """Initialize Learning Service-specific capabilities"""
        if SELF_AWARE_AVAILABLE:
            # Register event handlers with priorities
            self.register_handler("learning.training.*", self.handle_training_event, priority=EventPriority.HIGH)
            self.register_handler("learning.awareness.*", self.handle_awareness_event, priority=EventPriority.HIGH)
            self.register_handler("learning.competency.*", self.handle_competency_event, priority=EventPriority.NORMAL)
            self.register_handler("learning.knowledge.*", self.handle_knowledge_event, priority=EventPriority.NORMAL)

            logger.info("✅ Learning Service event handlers registered")

    async def handle_training_event(self, event):
        """Handle learning.training events"""
        logger.info(f"Handling learning.training event: {event.type}")
        # TODO: Implement actual training handling
        return {"status": "processed", "event_type": event.type}
    async def handle_awareness_event(self, event):
        """Handle learning.awareness events"""
        logger.info(f"Handling learning.awareness event: {event.type}")
        # TODO: Implement actual awareness handling
        return {"status": "processed", "event_type": event.type}
    async def handle_competency_event(self, event):
        """Handle learning.competency events"""
        logger.info(f"Handling learning.competency event: {event.type}")
        # TODO: Implement actual competency handling
        return {"status": "processed", "event_type": event.type}
    async def handle_knowledge_event(self, event):
        """Handle learning.knowledge events"""
        logger.info(f"Handling learning.knowledge event: {event.type}")
        # TODO: Implement actual knowledge handling
        return {"status": "processed", "event_type": event.type}

    async def handle_completion_event(self, event):
        """Handle completion events (triggers next steps)"""
        logger.info(f"Learning Service completion event: {event.type}")
...
async def lifespan(app: FastAPI):
    ...
    # === STARTUP ===
    logger.info("=" * 60)
    logger.info("🚀 Starting Learning Service (Integrated with Graceful Choreography)")
    logger.info("=" * 60)
    logger.info(f"📍 Port: 8110")
    logger.info(f"📋 ISO 22301 Clause: 7.2-7.3")
    ...
    try:
        ...
        if SELF_AWARE_AVAILABLE:
            ...
            service_self_aware = LearningServiceSelfAware()
            ...
            if platform.intelligent_router:
                ...
                await platform.intelligent_router.register_subscriber(
                    subscriber_id="learning_service",
                    event_pattern="learning.*", "training.*",
                    handler=service_self_aware.on_event,
                    capabilities={
                        "domains": ["learning", "bcm"],
                        "max_concurrent": 10,
                ...
                await platform.eventbus.subscribe("learning.*", "training.*", service_self_aware.on_event)
        ...
        logger.info("\n" + "=" * 60)
        logger.info("✅ Learning Service ready (with Graceful Choreography)")
        logger.info("=" * 60)
        logger.info(f"   🌐 Docs: http://localhost:8110/docs")
        logger.info(f"   📊 Metrics: http://localhost:8110/metrics")
        logger.info(f"   ❤️  Health: http://localhost:8110/health")
        logger.info("=" * 60 + "\n")

    except Exception as e:
        logger.error(f"❌ Failed to start Learning Service: {e}", exc_info=True)
        raise
...
app = FastAPI(
    title=getattr(settings, 'SERVICE_NAME', 'Learning Service'),
    description=f"""
    Learning & Awareness - ISO 22301 Clauses 7.2-7.3
