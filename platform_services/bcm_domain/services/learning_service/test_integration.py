"""
Test Integration - Learning Service
===================================
...
async def test_platform_integration():
    ...
    print("\n" + "=" * 70)
    print("Testing Learning Service Platform Integration")
    print("=" * 70 + "\n")
    ...
    try:
        platform = await init_platform(
            database_url="sqlite+aiosqlite:///./test_learning_service.db",
    ...
    try:
        ...
        class TestLearningService(SelfAwareService):
            def __init__(self):
                super().__init__(
                    service_name="learning_service_test",
                    capabilities=["learning.training", "learning.awareness"]
                )

            async def handle_test_event(self, event):
                return {"status": "handled", "event_type": event.type}

        test_service = TestLearningService()

        test_service.register_handler("learning.test.*", test_service.handle_test_event, priority=EventPriority.HIGH)
    ...
    if platform.intelligent_router:
        ...
        try:
            ...
            test_event = Event.create(
                event_type="learning.integration.test",
                data={"message": "Integration test event for Learning Service"},
...
async def main():
    ...
    try:
        ...
        if success:
            ...
            print("   2. Test API endpoints: http://localhost:8110/docs")
            print("   3. Check health: curl http://localhost:8110/health")
            print("   4. Check platform status: curl http://localhost:8110/api/platform/status")
            return 0
        else:
