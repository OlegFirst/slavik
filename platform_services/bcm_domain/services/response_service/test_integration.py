"""
Test Integration - Response Service
===================================
...
async def test_platform_integration():
    ...
    print("\n" + "=" * 70)
    print("Testing Response Service Platform Integration")
    print("=" * 70 + "\n")
    ...
    try:
        platform = await init_platform(
            database_url="sqlite+aiosqlite:///./test_response_service.db",
    ...
    try:
        ...
        class TestResponseService(SelfAwareService):
            def __init__(self):
                super().__init__(
                    service_name="response_service_test",
                    capabilities=["response.incident_detection", "response.activation"]
                )

            async def handle_test_event(self, event):
                return {"status": "handled", "event_type": event.type}

        test_service = TestResponseService()

        test_service.register_handler("response.test.*", test_service.handle_test_event, priority=EventPriority.HIGH)
    ...
    if platform.intelligent_router:
        ...
        try:
            ...
            test_event = Event.create(
                event_type="response.integration.test",
                data={"message": "Integration test event for Response Service"},
...
async def main():
    ...
    try:
        ...
        if success:
            ...
            print("   2. Test API endpoints: http://localhost:8080/docs")
            print("   3. Check health: curl http://localhost:8080/health")
            print("   4. Check platform status: curl http://localhost:8080/api/platform/status")
            return 0
        else:
