"""
Response Service - Integrated with Graceful Choreography
=========================================================

Enhanced Response Service with:
...
class ResponseServiceSelfAware(SelfAwareService if SELF_AWARE_AVAILABLE else object):
    """
    Response Service with Self-Aware capabilities
    ...
    def __init__(self):
        if SELF_AWARE_AVAILABLE:
            ...
            logger.warning("️  Response Service running in basic mode (Self-Aware not available)")
    ...
    async def handle_completion_event(self, event):
        ...
        logger.info(f"Response Service completion event: {event.type}")
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if SELF_AWARE_AVAILABLE:
            ...
            service_self_aware = ResponseServiceSelfAware()
            ...
            if platform.intelligent_router:
                ...
                await platform.intelligent_router.register_subscriber(
                    subscriber_id="response_service",
                    event_pattern="response.*", "incident.*",
                    handler=service_self_aware.on_event,
                    capabilities={
                        "domains": ["response", "bcm"],
                        "max_concurrent": 10,
                ...
                await platform.eventbus.subscribe("response.*", "incident.*", service_self_aware.on_event)
        ...
        logger.error(f" Failed to start Response Service: {e}", exc_info=True)
        raise
    ...
    logger.info("=" * 60)
    logger.info(" Response Service shutdown complete")
    logger.info("=" * 60)


# Create FastAPI app with enhanced lifespan
app = FastAPI(
    title=getattr(settings, 'SERVICE_NAME', 'Response Service'),
    description=f"""
    Incident Response - ISO 22301 Clause 8.4.3
