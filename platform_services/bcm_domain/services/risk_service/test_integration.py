...
async def test_platform_integration():
    ...
    print("\n" + "=" * 70)
    print("Testing Risk Service Platform Integration")
    print("=" * 70 + "\n")
    ...
    try:
        platform = await init_platform(
            database_url="sqlite+aiosqlite:///./test_risk.db",  # Use test DB
            redis_url="redis://localhost:6379/2",  # Use different Redis DB
    ...
    try:
        ...
        class TestRiskService(SelfAwareService):
            def __init__(self):
                super().__init__(
                    service_name="risk_service_test",
                    capabilities=["risk.test", "risk.assessment", "risk.analysis"]
                )

            async def handle_test_event(self, event):
                return {"status": "handled", "event_type": event.type}

        test_service = TestRiskService()

        # Register handler
        test_service.register_handler("risk.test.*", test_service.handle_test_event, priority=EventPriority.HIGH)
    ...
    if platform.intelligent_router:
        ...
        try:
            ...
            test_event = Event.create(
                event_type="risk.integration.test",
                data={"message": "Integration test event for Risk Service"},
...
async def main():
    ...
    try:
        ...
        if success:
            ...
            print("   2. Test API endpoints: http://localhost:8040/docs")
            print("   3. Check health: curl http://localhost:8040/health")
            print("   4. Check platform status: curl http://localhost:8040/api/platform/status")
            return 0
        else:
