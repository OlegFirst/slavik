"""
Test Integration - Validation Service
=====================================
...
async def test_platform_integration():
    ...
    print("\n" + "=" * 70)
    print("Testing Validation Service Platform Integration")
    print("=" * 70 + "\n")
    ...
    try:
        platform = await init_platform(
            database_url="sqlite+aiosqlite:///./test_validation_service.db",
    ...
    try:
        ...
        class TestValidationService(SelfAwareService):
            def __init__(self):
                super().__init__(
                    service_name="validation_service_test",
                    capabilities=["validation.exercise", "validation.test"]
                )
        ...
        test_service.register_handler("validation.test.*", test_service.handle_test_event, priority=EventPriority.HIGH)
    ...
    if platform.intelligent_router:
        ...
        try:
            ...
            test_event = Event.create(
                event_type="validation.integration.test",
                data={"message": "Integration test event for Validation Service"},
...
async def main():
    ...
    try:
        ...
        if success:
            ...
            print("   2. Test API endpoints: http://localhost:8100/docs")
            print("   3. Check health: curl http://localhost:8100/health")
            print("   4. Check platform status: curl http://localhost:8100/api/platform/status")
            return 0
        else:
