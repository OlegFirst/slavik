"""
Validation Service - Integrated with Graceful Choreography
===========================================================

Enhanced Validation Service with:
...
class ValidationServiceSelfAware(SelfAwareService if SELF_AWARE_AVAILABLE else object):
    """
    Validation Service with Self-Aware capabilities
    ...
    def __init__(self):
        if SELF_AWARE_AVAILABLE:
            ...
            logger.info(" Validation Service initialized as Self-Aware")
        else:
            logger.warning("️  Validation Service running in basic mode (Self-Aware not available)")

    async def initialize(self):
        """Initialize Validation Service-specific capabilities"""
        if SELF_AWARE_AVAILABLE:
            # Register event handlers with priorities
            self.register_handler("validation.exercise.*", self.handle_exercise_event, priority=EventPriority.HIGH)
            self.register_handler("validation.test.*", self.handle_test_event, priority=EventPriority.HIGH)
            self.register_handler("validation.review.*", self.handle_review_event, priority=EventPriority.NORMAL)
            self.register_handler("validation.improvement.*", self.handle_improvement_event, priority=EventPriority.NORMAL)

            logger.info(" Validation Service event handlers registered")

    async def handle_exercise_event(self, event):
        """Handle validation.exercise events"""
        logger.info(f"Handling validation.exercise event: {event.type}")
        # TODO: Implement actual exercise handling
        return {"status": "processed", "event_type": event.type}
    async def handle_test_event(self, event):
        """Handle validation.test events"""
        logger.info(f"Handling validation.test event: {event.type}")
        # TODO: Implement actual test handling
        return {"status": "processed", "event_type": event.type}
    async def handle_review_event(self, event):
        """Handle validation.review events"""
        logger.info(f"Handling validation.review event: {event.type}")
        # TODO: Implement actual review handling
        return {"status": "processed", "event_type": event.type}
    async def handle_improvement_event(self, event):
        """Handle validation.improvement events"""
        logger.info(f"Handling validation.improvement event: {event.type}")
        # TODO: Implement actual improvement handling
        return {"status": "processed", "event_type": event.type}

    async def handle_completion_event(self, event):
        """Handle completion events (triggers next steps)"""
        logger.info(f"Validation Service completion event: {event.type}")
...
async def lifespan(app: FastAPI):
    ...
    # === STARTUP ===
    logger.info("=" * 60)
    logger.info(" Starting Validation Service (Integrated with Graceful Choreography)")
    logger.info("=" * 60)
    ...
    try:
        ...
        if SELF_AWARE_AVAILABLE:
            ...
            service_self_aware = ValidationServiceSelfAware()
            ...
            if platform.intelligent_router:
                ...
                await platform.intelligent_router.register_subscriber(
                    subscriber_id="validation_service",
                    event_pattern="validation.*", "exercise.*", "test.*",
                    handler=service_self_aware.on_event,
                    capabilities={
                        "domains": ["validation", "bcm"],
                        "max_concurrent": 10,
                ...
                await platform.eventbus.subscribe("validation.*", "exercise.*", "test.*", service_self_aware.on_event)
        ...
        logger.info("\n" + "=" * 60)
        logger.info(" Validation Service ready (with Graceful Choreography)")
        logger.info("=" * 60)
        logger.info(f"    Docs: http://localhost:8100/docs")
        logger.info(f"    Metrics: http://localhost:8100/metrics")
        logger.info(f"   ️  Health: http://localhost:8100/health")
        logger.info("=" * 60 + "\n")

    except Exception as e:
        logger.error(f" Failed to start Validation Service: {e}", exc_info=True)
        raise
...
app = FastAPI(
    title=getattr(settings, 'SERVICE_NAME', 'Validation Service'),
    description=f"""
    Validation & Testing - ISO 22301 Clause 8.5
