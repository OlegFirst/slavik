"""
NICS Integration Client
========================

Integration with NICS (Next Generation Incident Command System) for advanced
BCM exercise management using ICS (Incident Command System) standards.

Adapted from: /simulation/exercise_simulators/nics_client.py
"""
...
class NICSRole(str, Enum):
    """NICS user roles for BCM exercises (ICS standard)"""
...
@dataclass
class NICSForm:
    """NICS form for structured reporting (ICS forms)"""
    id: str
    incident_id: str
...
class NICSClient:
    ...
    Provides integration with NICS for professional incident command
    and control following ICS (Incident Command System) standards.
    """

    def __init__(
        self,
        nics_url: str = "http://localhost:9001",
        api_key: str = "",
    ...
    async def authenticate(self) -> bool:
        ...
        try:
            response = await self.client.get(f"{self.nics_url}/api/v1/auth/verify")
            ...
            if response.status_code == 200:
                ...
                logger.error(f"NICS authentication failed: {response.status_code}")
                return False
    ...
    async def create_bcm_incident(self, incident: NICSIncident) -> str:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                raise Exception(f"Failed to create incident: {response.status_code} - {error_text}")
    ...
    async def add_incident_resource(self, incident_id: str, resource: NICSResource) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                error_text = await response.text()
                logger.error(f"Failed to add resource: {response.status_code} - {error_text}")
                return False
    ...
    async def submit_form(self, form: NICSForm) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                error_text = await response.text()
                logger.error(f"Failed to submit form: {response.status_code} - {error_text}")
                return False
    ...
    async def get_incident_status(self, incident_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                error_text = await response.text()
                raise Exception(f"Failed to get incident status: {response.status_code} - {error_text}")
    ...
    async def close_incident(self, incident_id: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                error_text = await response.text()
                logger.error(f"Failed to close incident: {response.status_code} - {error_text}")
                return False
...
class BCMNICSIntegration:
    ...
    Provides high-level BCM exercise management using NICS including:
    - Exercise creation with ICS structure
    - Resource management
    - Event injection
    - Form management
    - Exercise completion and reporting
    """
    ...
    async def inject_exercise_event(self, exercise_id: str, event: dict) -> bool:
