...
Adapted from: /simulation/engines/monte_carlo_engine.py
"""
...
class MonteCarloEngine(BaseSimulationEngine):
    ...
    Uses random sampling to analyze uncertainty and risk in BCM scenarios.
    """
    ...
    async def run(self) -> Dict[str, Any]:
        ...
        Parameters:
            ...
            - calculation: Python expression using variables
              (e.g., "recovery_time * 1000 + financial_loss")

        Returns:
            Statistical results with percentiles and confidence intervals
        """
        ...
        return {
            ...
            "confidence": 0.95,
            "distribution_type": self._infer_distribution_type(results)
        }
    ...
    def _generate_samples(self, variables: Dict[str, Any], iterations: int) -> Dict[str, np.ndarray]:
        ...
        for var_name, var_config in variables.items():
            ...
            if distribution == 'normal':
                ...
                samples[var_name] = np.random.normal(mean, std, iterations)
                logger.warning(f"Unknown distribution '{distribution}' for {var_name}, using normal")
    ...
    def _run_calculations(self, samples: Dict[str, np.ndarray], calculation: str, iterations: int) -> np.ndarray:
        ...
        for i in range(iterations):
            ...
            try:
                ...
                logger.error(f"Calculation error at iteration {i}: {e}")
                results.append(0)
    ...
    def _analyze_statistics(self, results: np.ndarray) -> Dict[str, float]:
        ...
        return {
            ...
            "coefficient_of_variation": float(np.std(results) / mean_val) if mean_val != 0 else 0,
    ...
    def _infer_distribution_type(self, results: np.ndarray) -> str:
        """Infer the resulting distribution type"""
        skewness = self._calculate_skewness(results)
